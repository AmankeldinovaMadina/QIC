...
def test_complete_workflow():
    ...
    try:
        trip_response = requests.post(
            f"{BASE_URL}/trips", json=trip_payload, headers=HEADERS
        )
        ...
        if trip_response.status_code == 200:
            ...
            ranking_payload = {
                ...
                "flights": [
                    ...
                    {
                        "id": "flight2",
                        "price": {"amount": 380.0, "currency": "USD"},
                        "total_duration_min": 330,
                        "stops": 0,
                        "legs": [
                            {
                    ...
                    {
                        "id": "flight3",
                        "price": {"amount": 420.0, "currency": "USD"},
            ...
            ranking_response = requests.post(
                f"{BASE_URL}/flights/ai-rank", json=ranking_payload, headers=HEADERS
            )
                ...
                if ranking_result.get("items"):
                    ...
                    if original_flight:
                        ...
                        selection_response = requests.post(
                            f"{BASE_URL}/flights/select",
                            json=selection_payload,
                            headers=HEADERS,
                        )
                        print(
                            f"Flight selection status: {selection_response.status_code}"
                        )
                        ...
                        if selection_response.status_code == 200:
                            ...
                            if selection_result.get("trip", {}).get("selected_flight"):
                                ...
                                print(
                                    f"Selected flight: {selected.get('airline')} {selected.get('flight_number')}"
                                )
                                print(
                                    f"Route: {selected.get('departure_airport')} → {selected.get('arrival_airport')}"
                                )
                                print(
                                    f"Price: ${selected.get('price')} {selected.get('currency')}"
                                )
                            ...
                            trip_get_response = requests.get(
                            ...
                            if trip_get_response.status_code == 200:
                                ...
                                if updated_trip.get("selected_flight"):
                                    ...
                                    print(
                                        f"Trip has flight: {trip_flight.get('airline')} {trip_flight.get('flight_number')}"
                                    )
                                else:
                                    print("❌ Flight not found in updated trip")
                            else:
                                print(
                                    f"❌ Failed to retrieve updated trip: {trip_get_response.status_code}"
                                )

                        else:
                            print(
                                f"❌ Flight selection failed: {selection_response.status_code}"
                            )
    ...
    try:
        ...
        if trip_response.status_code == 200:
            ...
            response = requests.post(
                f"{BASE_URL}/flights/select", json=selection_payload, headers=HEADERS
            )
